...
msa <- read.table("input_data_Copy.txt", sep="\t",  header=TRUE, check.names = F)
...
plot1 <- ggplot(melted_mat) + #add data and fill cells
  geom_tile(data = melted_mat, aes(x=factor(X2), y=X1, fill=value), colour = "black") + #add black border around cells
  geom_text(data = melted_mat, aes(x=factor(X2), y=X1,label = value), size=4) + 
...
df_new1 <- data.frame(matrix(0, ncol = x+1, nrow = 13))
...
mutation_summary <- ggtexttable(final_df, rows = NULL, theme = ttheme("light", padding = unit(c(3, 3), "mm"), tbody.style = tbody_style(fill = "white", size = 9), colnames.style = colnames_style(fill = "white", size = 9, color = "Black")))
mutation_summary1 <- mutation_summary %>% tab_add_title(text = main.title, face = "bold",  padding = unit(4, "line"))
